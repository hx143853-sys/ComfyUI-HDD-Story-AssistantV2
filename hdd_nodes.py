...
### 全局设定
- 风格: {style}
- 规则: 绝对去人名化（最终给我的提示词中就算含有人名也得是“名字（外貌描述）”），强制根据表描述角色全貌。
- 映射表:
{char_rules}
...
class HDD_Image_to_Video_Prompt_Gen:
    ...
    def generate_video_prompt(self, enable_batch_mode, file_name, model_select, api_key, style_tag, enable_reasoning, enable_sfx, enable_bgm, enable_dialogue, enable_test_mode, test_input_text, input_image=None, shot_number=1, image_directory=""):
        ...
        dialogue_instruction = "要求结合镜号内容和图片内容生成台词提示。分镜脚本中的台词必须原封不动地使用，不要修改台词内容，但可以添加声线描述，例如'一个低沉的成熟男人说道：...'、'一个清脆的少女声音说道：...'等。需要根据图片中的人物特征和分镜内容判断合适的声线描述。" if enable_dialogue else "禁止生成任何台词相关内容。"
...
[
  {
    ...
    "运镜/动效": "镜头推近",
    "音效/BGM": "沉闷的心跳声",
    "台词": "低沉的中年男人声音:如果给你一个机会,把家里那位黄脸婆的头换掉...这三款,你会选哪一个?"
  }
]
